...
export default function TestEnvironment({ testId, title = "MCQ Assessment", durationMinutes, warningsAllowed = 3, questions }: TestEnvironmentProps) {
    ...
    }, [answers, isSubmitted, router, testId, studentEmail, studentName, warnings])

    // Timer logic
    const [prevDuration, setPrevDuration] = useState(durationMinutes);
    if (durationMinutes !== prevDuration) {
    ...
    return (
        <div className="min-h-screen bg-slate-50 flex flex-col select-none">
            {/* Header */}
            <header className="bg-white border-b border-slate-200 px-4 md:px-6 py-3 md:py-4 flex flex-wrap md:flex-nowrap items-center justify-between gap-y-3 gap-x-2 sticky top-0 z-50 shadow-sm shrink-0">
                <h1 className="font-bold text-lg md:text-xl text-slate-800 truncate mr-1 max-w-[40%] md:max-w-xs" title={title}>{title}</h1>
                <div className="flex items-center gap-3 md:gap-6 shrink-0 ml-auto">
                    <div className="flex flex-col text-right">
                        <span className="text-xs md:text-sm text-slate-500 font-medium leading-tight">Time Remaining</span>
                        <div className={`flex items-center gap-1 md:gap-2 text-base md:text-xl font-bold font-mono ${timeLeft < 300 ? 'text-red-500' : 'text-slate-800'}`}>
                    ...
                    <button
                        ...
                        className="bg-sky-500 hover:bg-sky-600 text-white px-3 md:px-5 py-2 md:py-2.5 rounded-lg font-medium text-sm md:text-base transition-colors flex items-center gap-1.5 shadow-md shadow-sky-500/20 whitespace-nowrap shrink-0"
                    >
                        <CheckCircle className="w-4 h-4 hidden sm:block" /> Submit Test
                    </button>
                </div>
            </header>
                            ...
                            <div key={q.id} className="space-y-6 animate-in fade-in slide-in-from-right-8 duration-500">
                                ...
                                <div className="flex flex-col-reverse sm:flex-row items-center justify-between mt-12 pt-6 border-t border-slate-100 gap-4">
                                    <button
                                        ...
                                        className="flex w-full sm:w-auto justify-center items-center gap-2 px-5 py-2.5 rounded-lg font-medium transition-colors border border-slate-200 text-slate-600 hover:bg-slate-50 disabled:opacity-50 disabled:cursor-not-allowed"
                                    >
                                    ...
                                    {currentQuestionIndex < questions.length - 1 ? (
                                        <button
                                            ...
                                            className="flex w-full sm:w-auto justify-center items-center gap-2 px-6 py-2.5 rounded-lg font-semibold transition-colors bg-sky-50 text-sky-600 hover:bg-sky-100 ring-1 ring-sky-200"
                                        >
                                        ...
                                        <button
                                            type="button"
                                            onClick={() => submitTest('Manual Submission')}
                                            className="flex w-full sm:w-auto justify-center items-center gap-2 px-6 py-2.5 rounded-lg font-semibold transition-colors bg-sky-500 text-white hover:bg-sky-600 shadow-md shadow-sky-500/20"
                                        >
